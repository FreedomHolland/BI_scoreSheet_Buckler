...
// ROW 1
  ...
  // ROW 2
  worksheet.addRow({ A: 'Buckler Scoring Sheet' })
...
const generateTeamHeaders = (worksheet, tournamentType) => {
  ...
  const row5 = worksheet.addRow({
    ...
    D: 'Matches (M)',
    I: 'Rounds (R)',
    N: 'Hits earned(E) vs Hits received(Re)',
    T: 'Cards',
    W: 'Points',
    X: 'Placement',
    Y: 'Rank adj. Points',
  ...
  const row6 = worksheet.addRow({
    ...
    P: 'G',
    Q: 'Re per R',
    R: 'E|Re Diff', // active versus ground difference
...
const generateTeamDataRows = (worksheet, teams) => {
  teams.forEach((team, index) => {
    ...
    const addedRow = worksheet.addRow({
      ...
      R: { formula: `P${rowIndex}-N${rowIndex}` },
      S: { formula: `IFERROR(((N${rowIndex}*2) + (P${rowIndex}*1))/(L${rowIndex}*5),0)` },
